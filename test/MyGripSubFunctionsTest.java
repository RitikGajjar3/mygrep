...
public class MyGripSubFunctionsTest extends CommonTestUtil {
    ...
    @ParameterizedTest
    @MethodSource("MyGripSubFunctionsTest#getReadFileAndSearchWordInputAndOutputData")
    void readFileAndSearchWordTest_HappyFlow(List<String> expectedMatchedLineList, String searchString, String fileName) throws Exception {
        Assertions.assertEquals(expectedMatchedLineList, MyGrip.readFileAndSearchWord(searchString, fileName, false));
    }
    ...
    void readFileAndSearchWordTest_FileNotFound(){
        Assertions.assertThrows(FileNotFoundException.class, () -> MyGrip.readFileAndSearchWord(INPUT_SEARCH_STRING_1, TEST_INVALID_FILE_PATH, false));
    }
    ...
    void takeInputLinesFromConsoleTest_HappyFlow(){
        ...
        List<String> actualInputList = MyGrip.takeInputLinesFromConsole();
    ...
    void takeInputLinesFromConsoleTest_NoInput(){
        ...
        ByteArrayInputStream testIn = new ByteArrayInputStream(consoleInput.getBytes());
        System.setIn(testIn);
        Assertions.assertThrows(NoSuchElementException.class, MyGrip::takeInputLinesFromConsole);
    }
    ...
    void isStringPresentInInputTest_CaseInsensitive_HappyFlow(){
        Assertions.assertEquals(true,
                MyGrip.isSearchStringPresentInInput(INPUT_SEARCH_STRING_1, INPUT_SEARCH_STRING_1.toLowerCase(Locale.ROOT), true));
    }

    @Test
    void isStringPresentInInputTest_CaseInsensitive2_HappyFlow(){
        Assertions.assertEquals(true,
                MyGrip.isSearchStringPresentInInput(INPUT_SEARCH_STRING_1, INPUT_SEARCH_STRING_1,true));
    }

    @Test
    void isStringPresentInInputTest_CaseSensitive_HappyFlow(){
        Assertions.assertEquals(true,
                MyGrip.isSearchStringPresentInInput(INPUT_SEARCH_STRING_1, INPUT_SEARCH_STRING_1, false));
    }

    @Test
    void isStringPresentInInputTest_CaseSensitive2_HappyFlow(){
        Assertions.assertEquals(false,
                MyGrip.isSearchStringPresentInInput(INPUT_SEARCH_STRING_1, INPUT_SEARCH_STRING_1.toLowerCase(Locale.ROOT), false));
    }
    ...
    void writeDataIntoNewFileTest_HappyFlow() throws Exception {
        ...
        MyGrip.writeDataIntoNewFile(inputLines, newFileName);
    ...
    void writeDataIntoNewFileTest_FileAlreadyExist(){
        Assertions.assertThrows(FileAlreadyExistsException.class, () -> MyGrip.writeDataIntoNewFile(new ArrayList<>(), TEST_READ_FILE_PATH));
    }
}